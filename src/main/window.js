...
function loadFallbackProfile(win) {
  ...
  // I1 fix (whole-branch review): make the render session actually BE the
 ...
 *  - Otherwise: falls back to the Phase-1 bootstrap poller (existing behavior,
 *    no status pushes).
 * @param {BrowserWindow} win
 * @param {{ url:string, username?:string, password?:string }} connection - the
 *   dedicated viewport connection (2c) or the active profile (Phase-1 poller path)
...
function startViewportBridge(win, connection) {
  ...
  // ── Adoption mode (2c): native UCP device connection drives navigation ───────
  if (vp.url) {
    const client = new AdoptionClient();
    const deviceName = vp.name || `${os.hostname().toUpperCase()}_VIEWPORT`;
    // Push adoption state to the renderer overlay (Task 6 preload listens on
    ...
    // ── Fallback plumbing (2c): fatal error or no online within the grace
    ...
    client.on('error', (e) => {
      ...
      if (fatal) {
        ...
        if (!loadFallbackProfile(win)) {
          // I#1 (Task 6 review): a fatal error with no usable fallback must
  ...
  // ── Fallback: Phase-1 bootstrap poller (non-adoption) ─────────────────────────
 ...
 *    profile path below — see the 2c branch at the top of this function.
...
async function loadInitialPage(win, cliArgs = {}) {
  ...
  // ── Viewport mode: top-level override (2c) ──────────────────────────────────
  ...
  if (vp.enabled && !vp.url) {
    vlog('viewport enabled but no dedicated URL – using profile flow (Phase-1 poller if name set)');
  }
