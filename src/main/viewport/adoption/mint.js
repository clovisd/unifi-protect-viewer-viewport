 ...
 * `dataDir` is injected by the caller (Task 8/index.js supplies
 ...
 * itself has no internal timeout (flagged in Task 4 review, carried to
 * Task 7): if `tlsConnect` neither completes nor errors — a silent packet
 * drop, a firewalled host — its promise never settles and the caller would
...
function makeConnection(opts) {
  ...
  const wsFactory = async (url, { cert, key, headers }) => {
    ...
    // abort if it required the server to echo it). Live-spike finding.
